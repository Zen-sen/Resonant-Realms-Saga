...
async function main() {
  ...
  const bunnyFactory = await hre.ethers.getContractAt("HumanFactoryFacet", diamondAddress);

  console.log("🔥 Breathing life into Heitsi-Eibib (Human #1)...");
  ...
  try {
    const tx = await bunnyFactory.createGenesisHuman(genes, owner.address); 
    await tx.wait();
