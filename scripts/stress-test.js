...
async function main() {
  ...
  for (let i = 0; i <= 12; i++) {
    try {
      ...
      const tx = await factory.mintGenesisHuman(i);
      await tx.wait();
      console.log("✅");
    } catch (error) {
  ...
  const totalBunnies = await factory.getHumanCount(); // Assumes you have this getter
  ...
  for (let i = 0; i < totalBunnies; i++) {
    ...
    console.log(`Human #${i} | Tribe: ${tribeId.toString().padStart(2)} | Resonance: ${resonance}`);
  }
}
