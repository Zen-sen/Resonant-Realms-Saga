...
describe("🧬 Resonant Realms: Genetic Logic", function () {
  ...
  before(async function () {
    ...
    factory = await ethers.getContractAt("HumanFactoryFacet", diamondAddress);
  });

  it("Should breathe life into a Khoe-San Genesis (Tribe 0)", async function () {
    const tx = await factory.mintGenesisHuman(0);
    await tx.wait();

    const bunny = await factory.getHuman(2); // Index 2 because we already minted 0 and 1
  ...
  it("Should correctly identify Synthesis Trait for Tribe 12", async function () {
    const bunny = await factory.getHuman(1); // Our Coloured Tribe Human
    const tribeId = Number(bunny.genes & 0xFFFFn);
